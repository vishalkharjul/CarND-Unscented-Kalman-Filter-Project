 ...
 ******************************************************************************/
    ...
    VectorXd x_pred_mean_ = VectorXd(n_x_);

    //create covariance matrix for prediction
    MatrixXd P_pred_mean_ = MatrixXd(n_x_, n_x_);

    //predicted state mean
    x_pred_mean_.fill(0.0);
    for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //iterate over sigma points
        x_pred_mean_ = x_pred_mean_+ weights_(i) * Xsig_pred_.col(i);
    }

    //predicted state covariance matrix
    P_pred_mean_.fill(0.0);
    for (int i = 0; i < 2 * n_aug_ + 1; i++) {  //iterate over sigma points

        // state difference
        VectorXd x_diff = Xsig_pred_.col(i) - x_pred_mean_;
        //angle normalization
        while (x_diff(3)> M_PI) x_diff(3)-=2.*M_PI;
        while (x_diff(3)<-M_PI) x_diff(3)+=2.*M_PI;

        P_pred_mean_ = P_pred_mean_ + weights_(i) * x_diff * x_diff.transpose() ;
    }

    //print result
    //std::cout << "Predicted state" << std::endl;
    //std::cout << x_pred_mean_ << std::endl;
    //std::cout << "Predicted covariance matrix" << std::endl;
    //std::cout << P_pred_mean_ << std::endl;
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
    ...
    VectorXd y_new = z_ - (H_laser_ * x_);
    NIS_lidar_ = y_new.transpose() * Si *y_new;
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
    ...
    //convert measurement update in measurement space.

    VectorXd z_new_ =VectorXd(3);

    z_new_[0]=sqrt(pow(x_[0],2) + pow(x_[1],2));
    z_new_[1]=atan2(x_[1],x_[0]);


    if (fabs(z_new_[0]) < 0.0001) {
        z_new_[2] = 0;
    } else {
        z_new_[2] = (x_[0]*cos(x_[3])+x_[1]*sin(x_[3]))*x_[2]/z_new_[0];
    }

    VectorXd y_new = z_ - z_new_ ;
    while (y_new[1] > M_PI) {
        y_new[1] -= 2.* M_PI;
    }
    while (y_new[1] < -M_PI) {
    ...
    NIS_radar_ = y_new.transpose() * S_.inverse() *y_new;
